    ...
    return <section className="menu-group" style={{ padding: 16 }}>
        ...
        {(Object.keys(soundLabels) as SoundKind[]).map(kind => <div key={kind} style={{ padding: "12px 0", borderBottom: "1px solid var(--c-border, #eee)" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10, marginTop: 10 }}>
                <label style={{ position: "relative", display: "inline-flex", alignItems: "center", minHeight: 40, padding: "0 14px", borderRadius: 10, background: "var(--c-page-body-bg, #f0eef8)", fontSize: 13, fontWeight: 600, cursor: busy ? "wait" : "pointer", opacity: busy ? 0.5 : 1 }}>
                    {current[kind]?.asset ? "更换音频" : "上传音频"}
                    <input disabled={busy} type="file" accept="audio/*" aria-label={`${soundLabels[kind]}：上传音频`} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0, cursor: "pointer" }} onChange={e => { void uploadSound(kind, e.target.files?.[0]); e.target.value = ""; }} />
                </label>
                {current[kind]?.asset && <button type="button" style={{ minHeight: 40, padding: "0 12px", fontSize: 13 }} onClick={() => { stopPreview?.(); setStopPreview(() => playChatSound(kind, { [kind]: { ...current[kind], mode: "custom" } })); }}>试听</button>}
                {current[kind]?.name && <span style={{ flexBasis: "100%", fontSize: 12, opacity: 0.65, overflowWrap: "anywhere" }}>{current[kind]?.name}</span>}
            </div>
        </div>)}
